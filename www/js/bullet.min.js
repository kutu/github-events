...
"transport":function(){return fake}}}};var tn=0;function next(){var c=0;for(var f in transports){if(tn==c){var t=transports[f]();if(t){var ret=new t.transport(url);ret.heart=t.heart;return ret}tn++}c++}return false}var stream=new function(){var readyState=CLOSED;var heartbeat;var delay=delayDefault=80;var transport=next();function init(){readyState=CONNECTING;if(!transport)return false;transport.onopen=function(){delay=delayDefault;if(transport.heart)heartbeat=setInterval(function(){stream.onheartbeat()},
2E4);if(readyState!=OPEN){readyState=OPEN;stream.onopen()}};transport.onclose=function(){clearInterval(heartbeat);if(readyState==CLOSING){readyState=CLOSED;stream.onclose()}else{if(readyState==CONNECTING)tn++;delay*=2;if(delay>1E4)delay=1E4;setTimeout(function(){transport=next();init()},delay)}};transport.onerror=transport.onclose;transport.onmessage=function(e){stream.onmessage(e)}}init();this.onopen=function(){};this.onmessage=function(){};this.onclose=function(){};this.onheartbeat=function(){};
this.setURL=function(newURL){url=newURL};this.send=function(data){return transport.send(data)};this.close=function(){readyState=CLOSING;transport.close()}};return stream}})})(jQuery);
